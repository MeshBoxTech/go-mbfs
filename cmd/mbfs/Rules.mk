...
$(d)_flags =-ldflags="-X "mbfs/go-mbfs".CurrentCommit=$(git-hash)"

$(d)-try-build $(IPFS_BIN_$(d)): GOFLAGS += $(cmd/mbfs_flags)
...
$(d)-install: GOFLAGS += $(cmd/mbfs_flags)
$(d)-install: $(d) $$(DEPS_GO) ALWAYS 
	$(GOCC) install $(go-flags-with-tags) ./cmd/mbfs
.PHONY: $(d)-install
...
$(COVER_BIN_$(d)): $(d) $$(DEPS_GO) ALWAYS
	$(eval TMP_PKGS := $(shell $(GOCC) list -f '{{range .Deps}}{{.}} {{end}}' $(go-flags-with-tags) ./cmd/mbfs | sed 's/ /\n/g' | grep mbfs/go-mbfs) $(call go-pkg-name,$<))

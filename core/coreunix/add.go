...
import (
	...
	"mbfs/go-mbfs/gx/QmR6YMs8EkXQLXNwQKxLnQp2VBZSepoEJ8KCZAyanJHhJu/go-ipfs-posinfo"
	...
	"mbfs/go-mbfs/gx/QmXLCwhHh7bxRsBnCKNE9BAN87V44aSxXLquZYTtjr6fZ3/go-unixfs"
	"mbfs/go-mbfs/gx/QmXLCwhHh7bxRsBnCKNE9BAN87V44aSxXLquZYTtjr6fZ3/go-unixfs/importer/balanced"
	ihelper "mbfs/go-mbfs/gx/QmXLCwhHh7bxRsBnCKNE9BAN87V44aSxXLquZYTtjr6fZ3/go-unixfs/importer/helpers"
	"mbfs/go-mbfs/gx/QmXLCwhHh7bxRsBnCKNE9BAN87V44aSxXLquZYTtjr6fZ3/go-unixfs/importer/trickle"
	"mbfs/go-mbfs/gx/QmZMWMvWMVKCbHetJ4RgndbuEF1io2UpUxwQwtNjtYPzSC/go-ipfs-files"
	...
	"mbfs/go-mbfs/gx/QmcUXFi2Fp7oguoFT81f2poJpnb44dFkZanQhDBHMoYyG9/go-mfs"
...
func (adder *Adder) mfsRoot(acckey string) (*mfs.Root, error) {
	if adder.mroot != nil {
		return adder.mroot, nil
	}
	rnode := unixfs.EmptyDirNode()	// 创建一个 Data_Directory 类型的 ProtoNode
	rnode.SetCidBuilder(adder.CidBuilder)
	// added by vingo  在这里的根据传入的 acckey 给 rnode 赋值
	if acckey != ""{
		rnode.AccessKey = []byte(acckey)
	}
	//////////////////////
...
func (adder *Adder) add(reader io.Reader) (ipld.Node, error) {
	chnk, err := chunker.FromString(reader, adder.Chunker) // 生成文件切割器
	if err != nil {
		return nil, err
	}
...
func (adder *Adder) Finalize() (ipld.Node, error) {
	...
	// added by vingo
	rootdir.AccessKey = adder.AccessKey
	//////////////////
	root = rootdir
...
func (adder *Adder) outputDirs(path string, fsn mfs.FSNode) error {
	...
	case *mfs.Directory:
		...
		// added by vingo
		if len(fsn.AccessKey) <= 0 && len(adder.AccessKey) > 0{
			fsn.AccessKey = adder.AccessKey
		}
		///////////////////
...
func (adder *Adder) addNode(node ipld.Node, path string) error {
	...
	// 将 node 节点添加到 path 在 mr.dir 所对应的 Directory 的 unixfsDir.node.link 中
...
func (adder *Adder) AddAllAndPin(file files.File) (ipld.Node, error) {
	...
	switch {
		case file.IsDirectory():
			...
			// single files.File f is treated as a directory, affecting hidden file semantics.
			for {
				f, err := file.NextFile()
				if err == io.EOF {
...
func (adder *Adder) addFile(file files.File) error {
	...
	// 生成文件 file 切块后构建的 ProtoNode 赋给 dagnode
	...
	// patch it into the root
	// 将 dagnode 添加到 merkleDAG 树上去, 具体工作就是：
	// 如果adder.mroot 为空就新建一个 Root 结构赋给 adder.mroot，
	// 然后将 dagnode 节点添加到 adder.mroot.dir.unixfsDir.node.link 中
	return adder.addNode(dagnode, addFileName)
}

func (adder *Adder) addDir(dir files.File) error {
	log.Infof("adding directory: %s", dir.FileName())
	//fmt.Println("=========== adding directory: ", dir.FileName())
	//mr, err := adder.mfsRoot()
	// added by vingo
	mr, err := adder.mfsRoot("")
	//////////////////

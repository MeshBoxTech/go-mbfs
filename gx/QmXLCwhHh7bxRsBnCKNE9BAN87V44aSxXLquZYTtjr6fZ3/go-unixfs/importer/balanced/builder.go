...
func Layout(db *h.DagBuilderHelper) (ipld.Node, error) {
	...
	// 用读出的第一块数据创建一个 ProtoNode 叶子节点，并将其作为当前的 Root，在这过程中，会将数据进行 protobuf 编码
	...
	// 判断 db.spl 对应的 io.Reader 是否已经将文件数据读完，未完则继续读取数据并创建新的 Node
	// 并将前面第一块生成的 Root 作为其 child 节点进行 MerkleDAG 树的构建
	for depth := 1; !db.Done(); depth++ {

		// Add the old `root` as a child of the `newRoot`.
		newRoot := db.NewFSNodeOverDag(ft.TFile)	// 创建一个 FSNodeOverDag 节点
		newRoot.AddChild(root, fileSize, db)	    // 将前面的 root 节点挂到 newRoot.dag.link
	...
	// 将新构建的 root 节点添加到 db 这个 dag 构造器的 dserv.b.nodes 成员中,
	// 在这过程中，会计算这个 root 的 cid 了(这里添加需要加密的参数？)
	// added by vingo  对 root 节点编码后的数据加密
	r,ok := root.(*mdag.ProtoNode)
	if ok {
		r.AccessKey = db.AccessKey
		return r, db.Add(r)
	}
	/////////////////////////
...
func fillNodeRec(db *h.DagBuilderHelper, node *h.FSNodeOverDag, depth int) (filledNode ipld.Node, nodeFileSize uint64, err error) {
	...
	for node.NumChildren() < db.Maxlinks() && !db.Done() {
		...
		// 将 childNode 添加到 node.dag.links，将 childFileSize 赋值给 node.file.format.Filesize

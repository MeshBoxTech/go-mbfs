...
import (
	...
	ipldcbor "mbfs/go-mbfs/gx/QmRoARq3nkUb13HSKZGepCZSWe5GrVPwx7xURJGZ7KWv9V/go-ipld-cbor"
...
func (n *dagService) Session(ctx context.Context, key string) ipld.NodeGetter {
	return &sesGetter{bs: bserv.NewSession(ctx, n.Blocks)}
}
...
func FetchGraphWithDepthLimit(ctx context.Context, root cid.Cid, depthLim int, serv ipld.DAGService) error {
	...
	if ok {
		ng = &sesGetter{bs:bserv.NewSession(ctx, ds.Blocks)}
	}

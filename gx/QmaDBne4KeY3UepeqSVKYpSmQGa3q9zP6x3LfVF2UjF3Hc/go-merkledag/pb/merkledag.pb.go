...
import io "io"
import fmt "fmt"
import github_com_gogo_protobuf_proto "mbfs/go-mbfs/gx/QmdxUuburamoF6zF9qjeQC4WYcWGbWuRmdLacMEsW8ioD8/gogo-protobuf/proto"
...
func (m *PBNode) Unmarshal(data []byte) error {
	...
	for index < l {
		...
		case 3:
			...
			m.AccessKey = append([]byte{}, data[index:postIndex]...)
			index = postIndex
		////////////////
		case 2:
			if wireType != 2 {
...
func (m *PBNode) MarshalTo(data []byte) (n int, err error) {
	...
	if len(m.AccessKey) > 0  {
		data[i] = 0x18
		i++
		i = encodeVarintMerkledag(data, i, uint64(len(m.AccessKey)))
